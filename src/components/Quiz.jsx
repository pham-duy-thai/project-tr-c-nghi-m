...
useEffect(() => {
  ...
  return (
    <div>
      ...
      <p className="questions">{QuizData[currentQuestion].question}</p>
      ...
      {optionSelected === QuizData[currentQuestion].answer ? (
        ...
        <p className="incorrect-answer">Câu trả lời của bạn sai</p>
      )}


      <div className="nav-buttons">
        <button onclick={goBack}disabled={currentQuestion===0}>Quay Lại</button>
        <button onClick={goNext}disable={!optionSelected}>{
          currentQuestion=== QuizData.lenght-1?"Hoàn Thành":"Kế Tiếp"}</button>
      </div>
    </div>
  );
};
